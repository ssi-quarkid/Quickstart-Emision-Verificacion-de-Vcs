...
import IonPublicKeyModel from './models/ModenaPublicKeyModel';
 ...
 */
export default class ModenaKey {
  /**
   * Generates SECP256K1 key pair to be used in an operation.
   * Mainly used for testing.
   * @returns [publicKey, privateKey]
   */
  public static async generateEs256kDidDocumentKeyPair (input: { id: string, purposes?: ModenaPublicKeyPurpose[] }): Promise<[IonPublicKeyModel, JwkEs256k]> {
    const id = input.id;
    ...
    const [publicKey, privateKey] = await ModenaKey.generateEs256kKeyPair();
  ...
  public static async generateEs256kOperationKeyPair (): Promise<[JwkEs256k, JwkEs256k]> {
    const keyPair = await ModenaKey.generateEs256kKeyPair();
    return keyPair;
  }
